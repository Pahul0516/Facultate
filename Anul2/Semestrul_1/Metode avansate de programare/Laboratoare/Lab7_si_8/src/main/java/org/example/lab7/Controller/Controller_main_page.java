        ...
        chatListView.setCellFactory(param -> new ListCell<Message>() {
            ...
            protected void updateItem(Message message, boolean empty) {
                ...
                if (empty || message == null) {
                    ...
                    // Use an HBox to control alignment
                    HBox container = new HBox(messageText);

                    // Check if the message is from the current user
                    if (message.getIdFromUser() == id_user) {
                        container.setAlignment(Pos.CENTER_RIGHT); // Align to the right
                        container.setStyle("-fx-background-color: lightblue; -fx-padding: 10; -fx-background-radius: 10;");
                    } else {
                        container.setAlignment(Pos.CENTER_LEFT); // Align to the left
                        container.setStyle("-fx-background-color: lightgray; -fx-padding: 10; -fx-background-radius: 10;");
                    }

                    setGraphic(container);
                    this.setUserData(message.getIdMessage());
                }
            }
        });
