...
PRINT 'S-a modificat coloana [pret] din tabelul [Biscuit]'
GO

-- DROP PROCEDURE do_proc_2
CREATE PROCEDURE do_proc_2 AS
ALTER TABLE Salar
ADD CONSTRAINT chk_salarbrut_max CHECK (valoare_brut < 6000);
PRINT 'S-a adaugat constrangere pentru coloana [valoare_brut] din tabelul [Salar]'
GO
...
ALTER TABLE ComenziFinalizate
ADD CONSTRAINT CFid FOREIGN KEY(Cid) REFERENCES Client(ID_Client)
PRINT 'S-a a creat o legatura intre Clienti si Comenzi finalizate'
GO
...
PRINT 'S-a remodificat coloana [pret] din tabelul [Biscuit]'
GO

-- DROP PROCEDURE undo_proc_2
CREATE PROCEDURE undo_proc_2 AS
ALTER TABLE Produse
DROP CONSTRAINT chk_salarbrut_max
...
PRINT 'S-a a elimina legatura intre Clienti si Comenzi finalizate'
GO
