...
{
	controller contr;
	contr.contr_adauga_eveniment("Tenis", "pe camp", "miscare", "20");
	contr.contr_adauga_eveniment("Tenis", "pe camp", "miscare", "12");
	assert(contr.contr_get_repository(contr).repo_get_size() == 2);
	contr.contr_modifica_eveniment(1, "FOTBAL", "pe camp", "miscare", 2);
	assert(contr.contr_get_activitati(contr).get_all().size() == 0);
	vector<Eveniment> v;
	v = contr.filtrare(filtrare_durata, "10"); 
	assert(v.size() == 1);
	v = contr.filtrare(filtrare_durata, "1");
	assert(v.size() == 0);
	contr.sortare(sortare_durata);
	contr.contr_sterge_eveniment_dupa_id(1);
	assert(contr.contr_get_repository(contr).repo_get_size() == 1);
	contr.contr_adauga_eveniment("Fotbal", "pe camp", "miscare", "2");
	contr.contr_adauga_eveniment("Baschet", "pe camp", "miscare", "2");
	contr.contr_adauga_eveniment("Inot", "pe camp", "miscare", "2");
	bool ok = false;
	contr.adauga_activitati(contr.contr_get_repository(contr).get_all(), "Inot", ok);
